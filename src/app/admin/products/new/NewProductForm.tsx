...
export function NewProductForm({ categories }: { categories: Category[] }) {
  ...
  function handleFrNameChange(value: string) {
    setTranslation("fr", "name", value);
    const slug = value
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
  ...
  async function handleTranslate() {
    ...
    try {
      ...
      const data = await res.json() as { translations?: Record<string, TranslationFields>; error?: string };
      if (!res.ok) { setTranslateError(data.error ?? "Erreur de traduction"); return; }
      const tr = data.translations!;
      setForm((f) => ({
        ...f,
        translations: { fr: f.translations.fr, en: tr.en ?? f.translations.en, zh: tr.zh ?? f.translations.zh },
      }));
      setTranslated(true);
    } catch {
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      <h1 className="mb-6 text-xl font-bold text-gray-900">Nouveau produit</h1>

      {saved ? (
        <div className="rounded-xl border border-green-200 bg-green-50 p-6 text-center">
          <Check className="mx-auto h-10 w-10 text-green-500" />
          <p className="mt-2 font-semibold text-green-800">Produit enregistré</p>
          <p className="text-sm text-green-600 mt-1">Le produit a été créé avec succès.</p>
          <Link
            href="/admin/products"
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <section className="rounded-xl border border-gray-200 bg-white p-5 space-y-5">
              <div className="flex items-center justify-between">
                ...
                >
                  {translating ? <><Loader2 className="h-4 w-4 animate-spin" />Traduction…</> :
                   translated ? <><Check className="h-4 w-4" />Traduit</> :
                   <><Sparkles className="h-4 w-4" />Traduire avec Claude</>}
                </button>
              </div>
              ...
              {translated && (
                <p className="rounded-lg bg-purple-50 px-3 py-2 text-sm text-purple-700">
                  ✓ Traduit en anglais et chinois. Vérifiez et corrigez si nécessaire.
                </p>
              )}
              ...
              {(["fr", "en", "zh"] as Locale[]).map((locale) => (
                <div key={locale} className="space-y-3">
                  <h3 className="text-xs font-semibold text-gray-500 uppercase">
                    {LOCALE_LABELS[locale]}{locale === "fr" && " (source)"}
                  </h3>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => locale === "fr" ? handleFrNameChange(e.target.value) : setTranslation(locale, "name", e.target.value)}
                      placeholder={locale === "fr" ? "Nom du produit en français" : ""}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-amber-400 focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder={locale === "fr" ? "Description détaillée, spécifications, délais…" : ""}
            ...
            <div className="flex gap-3">
              ...
              >
                {saving ? <><Loader2 className="h-4 w-4 animate-spin" />Enregistrement…</> : "Enregistrer le produit"}
              </button>
              <Link
                href="/admin/products"
