...
// Summary counts by status group
const groups = [
...
export default function AdminOrdersPage() {
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-xl font-bold text-gray-900">Commandes ({MOCK_ORDERS.length})</h1>

      {/* Summary cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {groups.map((g) => {
          const count = MOCK_ORDERS.filter((o) => g.statuses.includes(o.status)).length;
          return (
              ...
              <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  ...
                  {order.items.length > 1 && (
                    <p className="text-xs text-gray-400">+{order.items.length - 1} autre(s)</p>
                  )}
                </td>
                <td className="px-4 py-3">
                  <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${order.type === "AGENT" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}>
                    {order.type}
                  </span>
                </td>
